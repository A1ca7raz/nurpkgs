...
in {
  ...
  # locale.enable = true;
  bubblewrap.env.LOCALE_ARCHIVE = "/run/current-system/sw/lib/locale/locale-archive";
  ...
  bubblewrap = {
    ...
    bind.rw = [
      # TODO: add app.commonName
      [
        (sloth.concat' sloth.homeDir "/.local/state/nixpak/${config.flatpak.appId}/config")
        (sloth.concat' sloth.homeDir "/.config")
      ]
      [
        (envSuffix "HOME" "/.cache/${config.flatpak.appId}")
        sloth.xdgCacheHome
      ]
